...
describe('DeepCompositeSymbol', () => {
  ...
  it.skip("doesn't equal structurally different object", () => {
    expect(getDeepSymbol({ a: 1, b: 2 })).not.toStrictEqual(getDeepSymbol({ a: 1, b: 3 }));
  });

  it.skip('supports deep structural equality', () => {
    const o = () => ({ a: { c: 1 }, b: 2 });
    expect(getDeepSymbol(o())).toStrictEqual(getDeepSymbol(o()));
    expect(getDeepSymbol(o())).not.toStrictEqual(getDeepSymbol({}));
  });
});
